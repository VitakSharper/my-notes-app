...
| `webapp` | Next.js App Router client: HeroUI, Tailwind v4, Zustand, date-fns |
...
- `createdAt` is serialised without a `Z` even though the server stores UTC, so the browser reads it
  as local time and relative dates are off by the timezone offset. The fix belongs in the service
  (an EF Core converter that specifies `DateTimeKind.Utc`), not in the client.
- Viewing a question stamps `UpdatedAt`: the `ViewCount` increment goes through the generic
  `UpdateAsync`, so the detail page shows "modified today" on questions nobody edited.
