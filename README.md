...
MinIO values in `.env`, or a missing hosts entry. Deleting the data takes
...
The client app is part of the compose file — `PublishAsDockerFile()` builds it from
`webapp/Dockerfile` — and is served through nginx-proxy at https://app.overflow.local. Its
container also publishes host port 4000, but the app only really works through the proxy: the
Keycloak redirect URIs and `AUTH_URL` are both built for `app.overflow.local`.
...
In this mode an `nginx-proxy` container routes by `Host` header instead of publishing ports, so
every `VIRTUAL_HOST` it fronts has to be in your hosts file — all four of them:

```
127.0.0.1 api.overflow.local
127.0.0.1 id.overflow.local
127.0.0.1 app.overflow.local
127.0.0.1 minio.overflow.local
```

`minio` is the one that is easy to miss, because nothing fails to start without it: the browser
loads image URLs straight out of the question HTML, so the images simply never appear.

| | URL |
| --- | --- |
| Client app | https://app.overflow.local |
| Gateway | https://api.overflow.local |
| Keycloak (including the admin console) | https://id.overflow.local |
| MinIO (images) | https://minio.overflow.local/overflow |
| Aspire dashboard | http://localhost:8080 |

nginx-proxy terminates TLS with an mkcert certificate from `infra/dev-certs`; it drops the leftmost
label when looking one up, so `overflow.local.crt` covers all four names. Plain HTTP keeps working
on the same names (`HTTPS_METHOD=noredirect`) on purpose — `next dev` runs on the host and Node's
fetch ships its own CA bundle, so it ignores the Windows store where the mkcert root lives and
would refuse the certificate the browser accepts.
